...
NOTA sobre el anti-loop de repetición: el ejemplo oficial de Docling asigna
``custom_stopping_criteria = [DocTagsRepetitionStopper(N=32)]`` para cortar
los loops del modelo 258M con textos repetitivos. Con docling 2.126.0 eso
rompe al calcular el hash de caché de opciones
(``create_pipeline_options_hash`` → ``model_dump_json`` no serializa el
stopper, que no es pydantic). Es un bug upstream (el ejemplo oficial del repo
también lo usa). Hasta que lo arreglen, esta demo corre SIN el stopper; el
modelo ya trae ``stop_strings`` (``</doctag>``, ``<|end_of_text|>``). Para
reactivarlo cuando esté resuelto, descomentar en ``configurar_granite_docling()``:

    from docling.models.utils.generation_utils import DocTagsRepetitionStopper
    custom_vlm_options.custom_stopping_criteria = [DocTagsRepetitionStopper(N=32)]
...
def configurar_granite_docling() -> DocumentConverter:
    ...
    # Spec del modelo Granite Docling (descarga desde Hugging Face).
    custom_vlm_options = vlm_model_specs.GRANITEDOCLING_TRANSFORMERS.model_copy()

    # NOTA: el anti-loop DocTagsRepetitionStopper se omite por el bug de
    # serialización del hash en docling 2.126.0 (ver docstring del módulo).

    # Pipeline VLM con esas opciones.
    pipeline_options = VlmPipelineOptions(
        vlm_options=custom_vlm_options,
    )

    # Convertidor mapeando PDF e imagen al pipeline VLM.
