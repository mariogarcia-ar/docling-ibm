...
| **Estado de diseño** | 🟡 Motor declarativo (F0) + R1-R7 y contexto tipado (F3/T-301) + evidencia de lectura (F3/T-302) + reglas raw por fuente (F3/T-303, extendidas en F4/T-403) + precedencia por campo (F4/T-404, ejercitada por la paridad de F4/T-405) implementados; cruzadas/gaps (F5) pendientes |
...
| Tabla de precedencia por campo (`rules/precedencia.py`, PREC_n) | §6 + ADR-002 | E-EXT-1 | F4 / T-404 | [x] hecho: `TABLA_PRECEDENCIA` declarativa (`PREC_1` visual, `PREC_2` textual, `PREC_3` programa, `PREC_0` regla de oro), `resolver_campo()` (acuerdo/desacuerdo/ganadora invalidada/ninguna utilizable) y `combinar()` que conserva todas las lecturas |
...
| 2026-09-10 | T-404 (F4): `rules/precedencia.py` agrega la **tabla de precedencia por campo** de ADR-002, como `PrecedenciaCampo` declarativo por campo (`regla` + `motivo` legible) y no como reglas del motor: la precedencia no es una condición booleana sobre un contexto, es un **orden** entre fuentes que se aplica al resolver. `resolver_campo()` cubre los cinco casos (acuerdo, desacuerdo, una sola fuente, ninguna, y ganadora invalidada por la pasada 1) y `combinar()` arma los `CampoCombinado` conservando **todas** las lecturas. Hallazgo del diseño: el orden de las fuentes no puede ser una lista plana por campo — las fuentes que **no son lectura** (`hitl`/`arca`/`programa`) tienen que ir siempre por delante y la tabla solo debe decidir el orden **entre lecturas**; de ahí `orden_lecturas()` vs. `orden_completo()`. `VeredictoRaw.reglas_por_campo` (en `raw.py`) existe para que la resolución pueda decir **con qué regla** quedó débil la fuente que no gana. | team implementation | Hecho |
| 2026-09-10 | T-405 (F4): la paridad con v1 **no** agrega reglas al motor; ejerce lo que ya está. Los tests de integridad (`tests/test_extraction_paridad.py`) verifican que la tabla de precedencia cubra los campos del contrato (cada `PrecedenciaCampo` declara `regla` y `motivo`) y que la procedencia de las reglas raw siga trazable. Hallazgo colateral de la paridad — `proveedor` (clave obligatoria de `kvg`) no tenía normalizador y conservaba los espacios múltiples del OCR — se resolvió **en el llamador** (`extraction/key_value.py`: `"proveedor": NORM_TEXTO` + `CAMPOS_GENERICOS_CON_REGLA`), no en `rules/`, que sigue agnóstico del dominio: el motor no sabe qué es un proveedor. | team implementation | Hecho |
