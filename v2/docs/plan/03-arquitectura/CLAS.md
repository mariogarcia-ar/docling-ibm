...
| **Estado de diseño** | 🟡 T-301 (motor R1-R7), T-302 (prompt de evidencia + lector), T-303 (reglas raw por fuente) y T-304 (cadena contable 01→02→03) implementados; T-305 pendiente |
...
| 2026-09-10 | T-304 implementado: la **cadena contable 01→02→03** pasa de `v1/classification_pipeline.py` a la librería con **contratos tipados entre pasos** (cada paso es una dataclass inmutable y el siguiente consume el **primer** resultado del anterior, sin inventar su entrada). Los tres prompts de v1 se portan **literales** y quedan versionados (`contable-0X@1`, ADR-005); los checkpoints `<doc>_classification.json` conservan el nombre y el shape de v1 (T-305 compara contra ellos) con reanudación que **no** vuelve a llamar al modelo; `ErrorCadenaContable` preserva los resultados parciales. `clasificar_contable()` deja de ser un esqueleto y es la **variante pura** (sin red) sobre los pasos ya resueltos, con los mismos accesores que la cadena real para que no puedan divergir; `api.classify()` queda implementado. | team implementation | Hecho |
