...
| Campo | Valor |
|---|---|
| **Fase en curso** | **F5 — Conclusión + HITL** (T-501..T-507) · T-501..T-505 ✅ hechas |
...
| **F5** | 🟡 En implementación (T-501..T-505 ✅: reglas cruzadas, búsqueda acotada, consolidación, escalado al agente IA y cola HITL con muestreo de auditoría; T-506..T-507 pendientes) |
...
| **Suite de tests** | ✅ **1282 tests en verde + 10 skipped** (`python -m pytest --no-header -p no:cacheprovider`) en env `py313_env` |
...
| Conclusión (F5) | `conclusion/engine.py` + `conclusion/hitl.py` | `concluir()`/`concluir_caso()` (**T-501**), `concluir_con_busqueda()` (**T-502**), `consolidar_caso()` (**T-503**), `concluir_con_agente()`/`escalar_a_agente()` (**T-504**) y `encolar_hitl()` + `ColaHitl` (**T-505**) implementadas. Queda esqueleto `trace/recorder.py::CaseRecorder.registrar()` (T-506) |
...
> de conclusión (aprobado por negocio; rechazado por B con IVA; rechazado por A
> sin CUIT receptor; revisión por gap; revisión por R7; revisión por negocio que
> espera otra letra; revisión por tique `090`) más **8/8** fronteras. `--tabla`
> muestra solo la tabla y `--manual` el caso ambiguo, que es la forma de ver la
> conclusión **sin GPU**.
>
> **Decisión de alcance (T-501)**: **"está mal" y "no sé" son cosas distintas**.
> El fast-fail (`CRUZ_3`) **rechaza** con certeza alta —el código sí resolvió, y
> resolvió que no— mientras R7 solo **sospecha** y pide un ojo humano. Y el caso
> ambiguo **no se aprueba por descarte**: si nada concluye, queda en `revision`
> para el agente (T-504) o el HITL (T-505).
>
> **Hallazgo de contrato (T-501)**: el validador de `CombinedEvidence` exige que
> `origen=programa` implique `certeza=alta`, así que el `Decision` de F0 es el
> veredicto **final** y solo se adjunta cuando el código concluyó. Un caso ambiguo
> viaja **sin** `Decision` —no lo decidió nadie todavía— y su estado completo vive
> en el `ConclusionResult`. El contrato de F0 **rechazó** el primer diseño (que
> emitía un `Decision` de programa con certeza baja) y eso fue lo que motivó la
> separación.
...
### 3.2 Suite de tests (1282 en verde + 10 skipped)
```bash
cd v2
...
- ✅ **Conclusión: reglas cruzadas, búsqueda, consolidación, agente y HITL (T-501..T-505)** — la pasada 2 decide el caso, busca evidencia **acotada** si faltan datos, **consolida** el `VoucherResult`, escala al **agente IA** (con blindaje post-agente) cuando el código no pudo, y **encola a revisión humana** lo que no quedó de certeza alta (prioridad alta) más un **muestreo reproducible** de los que sí (prioridad baja), registrando las correcciones como **feedback separado por motivo** (R-03/R-09). ❌ Falta el resto de F5: `CaseRecord` persistido (T-506) y métricas (T-507).
...
### 3.9 Comprobación rápida de F5 (T-501..T-505)
...
python scripts/F5/t503.py                      # tabla de la regla + 6 escenarios de consolidación + 8 fronteras
...
python scripts/F5/t505.py                      # política HITL + 6 escenarios de encolado + 13 fronteras
python scripts/F5/t505.py --muestreo           # el muestreo, en números
python scripts/F5/t505.py --caso alta_muestreada       # auditoría con prioridad baja
...
python -m pytest tests/test_conclusion_cruzadas_t501.py tests/test_conclusion_gaps_t502.py \
    ...
    tests/test_conclusion_hitl_t505.py -q
```

```python
from voucherflow.conclusion import concluir_con_busqueda, consolidar_caso

# La vía corta: concluir (T-501) + consolidar (T-503) en una llamada.
resultado = consolidar_caso(combinada, contexto_tipo=ctx_fiscal)
resultado.valor.estado          # aprobado | rechazado | revision
resultado.valor.certeza         # alta si el código concluyó de forma consistente
resultado.valor.origen          # programa; None si el caso quedó ambiguo
resultado.motivo_certeza        # POR QUÉ esa certeza (auditoría E-CONC-5)

# El flujo completo: concluir → buscar evidencia acotada (T-502) → consolidar.
resultado = concluir_con_busqueda(
    combinada,
    buscador=ArcaClient(url="https://wscdc.afip.gob.ar"),
...
> solo si** el código concluyó de forma consistente: `concluye` ∧ sin alertas
> pendientes ∧ con letra.
>
> **Un rechazo firme también es certeza alta.** Es contraintuitivo pero es lo
> correcto: "esto no es válido" es una **conclusión**, no una duda. La certeza
> mide cuánto sabe el sistema, no si le gustó el resultado. Lo que **sí** baja la
> certeza es una **alerta de R7 sin resolver**: el Gherkin pide que las reglas
> concluyan "de forma consistente".
>
> **Un caso ambiguo sale sin `origen`**: no lo decidió nadie todavía, así que
> afirmar `programa` sería mentir sobre quién decidió. Es la señal para T-504
> (agente) y T-505 (HITL), no un resultado final.
>
> **La clasificación contable no se inventa**: sin los pasos resueltos de F3/T-304
> el resultado viaja **sin** ella y la traza lo declara con su motivo.
...
combinada = combinar_evidencia("doc-1", evidencias)   # F4/T-404 (decision=None)

# La pasada 2 (T-501) mira el valor vigente de cada campo y decide el caso.
resultado = concluir_caso(combinada)
resultado.estado          # 'aprobado' | 'rechazado' | 'revision'
resultado.certeza         # 'alta' si concluyó el código; None si queda ambiguo
resultado.reglas_aplicadas  # ['CRUZ_1'] / ['CRUZ_3'] / ['CRUZ_4'] / ['CRUZ_5']
resultado.faltan_datos    # los campos críticos que faltan (insumo de T-502)

# Con el contexto fiscal (lo aporta F3/padrón), además detecta R7 y el cruce
# negocio-vs-documento. Es opcional: sin él, la coherencia del documento alcanza.
ctx = ContextoTipoComprobante(
    emisor_condicion_fiscal="Responsable Inscripto",
    receptor_condicion_fiscal="Responsable Inscripto",
)
concluir_caso(combinada, ctx).alertas    # R7: RI+RI con una B → revisión
```

> **El caso ambiguo viaja sin `Decision`**, a propósito: el `Decision` de F0
> registra *quién* decidió y el contrato exige que `origen=programa` implique
> `certeza=alta` (glosario §2). Si el código no pudo concluir, todavía no decidió
> nadie: el estado completo viaja en el `ConclusionResult` y lo resolverán el
...
(F4/T-404), la paridad con v1 (F4/T-405), ni `concluir`/`concluir_caso`
(F5/T-501), `concluir_con_busqueda` (F5/T-502), `consolidar_caso`
(F5/T-503), `escalar_a_agente`/`concluir_con_agente` (F5/T-504) ni
`encolar_hitl`/`ColaHitl` (F5/T-505). **F4 queda cerrada** con su DoD
verificado y **F5 está en curso**; dentro de F5 el único esqueleto que queda
es `trace/recorder.py::CaseRecorder.registrar()` (T-506), cuya firma ya está
congelada desde F0.
