...
| Campo | Valor |
|---|---|
| **Fase en curso** | **F6 — Cliente (CLI/batch) e integración final** 🟡 **en implementación** (T-601 y T-602 hechas; T-603..T-606 pendientes) |
...
| **F6** | 🟡 En implementación (T-601: CLI `voucherflow` + orquestador + fachada; T-602: batch con workers, checkpoints/reanudación y enfriamiento ADR-010; faltan agregado T-603, paridad T-604, docs T-605 y API HTTP T-606) |
...
| **Suite de tests** | ✅ **1506 tests en verde + 10 skipped** (`python -m pytest --no-header -p no:cacheprovider`) en env `py313_env` |
...
python scripts/F5/t506.py                      # contrato + 4 escenarios de persistencia + 14 fronteras
...
`encolar_hitl`/`ColaHitl` (F5/T-505) ni `construir_case_record`/`CaseRecorder`
(F5/T-506). **F4 queda cerrada** con su DoD verificado y **F5 está en
curso**; sin esqueletos de F5 salvo lo que pertenece a **F6** (`api.run()` y
`PipelineOrchestrator.ejecutar()`, la CLI/batch), cuyas firmas ya están
congeladas desde F0.
