...
| **Estado de diseño** | ✅ **Congelado** (F4 completa, T-401..T-405: flujos, evidencia, normalización, reglas raw, combinación por campo y paridad con v1) |
...
| Combinar evidencia por campo con fuente | §4.4 + §6 (nota de diseño) | E-EXT-1 | F4 / T-404 | [x] hecho (`extraction/flows.py::combinar_evidencia` + `rules/precedencia.py`: conserva las lecturas y agrega `valor`/`fuente`/`resolucion` por campo) |
| Resolución por campo (`FieldResolution` con precedencia ADR-002) | §6 | E-EXT-1 | F4 / T-404 | [x] hecho (`rules/precedencia.py`: `TABLA_PRECEDENCIA` con `PREC_0`..`PREC_3`, `resolver_campo()` y la regla de oro —lo comprobado manda; entre lecturas, el papel) |
...
| Evidencia combinada (`CombinedEvidence`) como entrada de la conclusión | §4.5 + §9 | E-EXT / E-CONC | F4 → F5 / T-404→T-501 | [x] contrato listo (`combinar_evidencia` devuelve el `CombinedEvidence` con la resolución por campo y `decision` en `None`: la decisión la produce F5/T-501) |
| Clase documental publicada por el gate (`es_comprobante`) | §4.5 (fast-fail) | E-QWE / E-EXT | F2 / T-202 → F6 / T-601 | [x] hecho (`orchestrator._fuente_de_clase()` publica el veredicto del gate como fuente `programa` en la combinación del run **aprobado**). ⚠️ **No** está en `CAMPOS_EXTRACCION`: es una decisión del código, no una lectura del prompt de extracción (ADR-001). El campo y el vocabulario (`schemas.evidence.ClaseDocumento`) son los mismos que los del laboratorio de LLM externos, para que las dos puntas sean comparables |
| Paridad con `extraction_pipeline.py` (10/11) y `document_extraction.py` (kvi/kvg) | §8.2 (mapeo v1→v2) | E-EXT | F4 / T-405 | [x] hecho (`tests/golden/F4/` + `scripts/F4/paridad_extraccion.py` + `scripts/F4/t405.py`: paridad en tres niveles — normalización determinista, contrato/sostén y corrida real contra v1; reglas **20/20**, campos **29/29**, sostén **32/32**, genérico **5/5**) |
...
- [x] Interfaz pública acordada: firmas de los flujos VLM/LLM devolviendo `SourceEvidence` (`extraction/flows.py::flujo_vlm`/`flujo_llm`, T-401) y de la combinación devolviendo `CombinedEvidence` (`extraction/flows.py::combinar_evidencia`, **implementada en T-404** con la firma congelada de F0). El lector de modelo es un parámetro inyectable (protocolo `extraction.Lector`, mismo criterio que F3-subplan §2.6).
...
- [x] ADR(s) asociado(s) resueltos: ADR-001 (schema estricto por campo y prompts reescritos a evidencia: `extraccion-key-value@1` no normaliza ni decide — **la normalización vive en código**, `key_value.py`, T-402) y ADR-002 (**aplicado**: T-401 conserva ambas evidencias sin colapsar y `rules/precedencia.py` + `combinar_evidencia()` de T-404 aplican la tabla de precedencia por campo). **ADR-006** se materializa en T-403: la pasada 1 por fuente (contrato verificado en código, no pedido al prompt) incluye el sostén por forma canónica y la coherencia de la fuente consigo misma.
- [x] Casos de golden set / tests que lo validan: los casos de discrepancia VLM/LLM y de fuente inconsistentemente sostenida están cubiertos por `tests/test_extraction_flujos.py` + `scripts/F4/t401.py`, las reglas de normalización por `tests/test_extraction_key_value.py` (97) + `scripts/F4/t402.py`, la pasada 1 por `tests/test_extraction_raw_t403.py` (37) + `scripts/F4/t403.py` (11/11 criterios de sostén · 7/7 escenarios · 4/4 fronteras) y la combinación por `tests/test_extraction_combinacion_t404.py` (45) + `scripts/F4/t404.py` (8/8 + 4/4). El **subconjunto del golden y la paridad de campos normalizados contra v1** los aporta **T-405**: `tests/golden/F4/subconjunto.json` (7 reglas con procedencia + 6 casos), `tests/test_extraction_paridad.py` (39) y `scripts/F4/t405.py` (reglas **20/20** · campos **29/29** · sostén **32/32** · genérico **5/5**).
...
- **ADR-002 (D-2, bloqueante)** — Tabla de precedencia por campo: sin ella la combinación no resuelve desacuerdos de forma determinista; requiere workshop con negocio por tipo de campo. **Aplicado en T-404** (`rules/precedencia.py::TABLA_PRECEDENCIA`): T-401 conserva las dos evidencias y la combinación resuelve campo a campo con la tabla.
...
- **D-11 (sin ADR)** — Modalidades `llm`/`vlm`/`auto` y su mapeo a los nuevos flujos; afecta la paridad con los modos `kvi/kvg/10/11` de v1 (E-EXT-1 "modalidades de la librería v1"). **Resuelto en la práctica**: T-401 corre siempre las dos fuentes (no hay modalidad que elija una), su intérprete tolera el JSON plano de `kvi`/`kvg` y T-405 midió la paridad sobre el subconjunto del golden. La vía reproduce los modos de v1 mediante los dos flujos en paralelo, no mediante una modalidad exclusiva.
- **R-02 (riesgo)** — La migración de prompts (10/11/kvi/kvg) puede degradar calidad de extracción; mitigado con el golden set y la paridad por fase. **T-405 verifica la mitigación en tres niveles** (normalización determinista 20/20, contrato/sostén 32/32, corrida real contra v1 en `--subset origen`) y documenta las diferencias esperadas por diseño (`tipo_comprobante`, `moneda`, `descripcion`).
...
| 2026-09-10 | **T-404 hecha**: la **combinación con resolución por campo** (ADR-002) cierra E-EXT-1. `rules/precedencia.py` declara la tabla (`PREC_1` visual para la letra del recuadro, el número impreso, el membrete y los CUIT; `PREC_2` textual para fecha, moneda e importes; `PREC_3` programa para los derivados de T-402; `PREC_0` regla de oro para el modo genérico) y resuelve el campo respetando la pasada 1 de T-403. `combinar_evidencia()` devuelve el `CombinedEvidence` **conservando todas las lecturas**, con la resolución (`ganador`/`regla`/`motivo`), el atajo `valor`/`fuente` y la traza de la combinación; `decision` pasa a ser **opcional** porque combinar no es decidir (eso es F5/T-501), y `CampoCombinado` gana `valor`/`fuente` como atajo operativo. Las fuentes que no son lectura (`hitl` > `arca` > `programa`) van siempre por delante. | team implementation | Hecho |
| 2026-09-10 | **T-405 hecha — F4 cerrada**: la **paridad con v1** (10/11 y kvi/kvg) queda medida en **tres niveles** y el módulo pasa a ✅ **Congelado**. Nivel 1 (determinista, sin modelos): cada regla v2 aplicada al `texto_v1` literal del subconjunto produce el mismo canónico y **cita su origen** (`regla_v1` + `prompt_v1` + `texto_v1`) — **20/20**. Nivel 2: los 15 campos de paridad existen en el contrato F0 y el sostén estructurado se evalúa donde corresponde — **29/29** campos (**32/32** de sostén) y **5/5** en el modo genérico. Nivel 3 (con red, fuera de la suite default): corrida real contra v1 sobre 3 documentos del golden, informativa. Las **diferencias por diseño** (`tipo_comprobante` con el vocabulario `090`/`099` de v1, `moneda` sin el default `ARS`, `descripcion` normalizada en código) viajan como nota, no como fallo. **Hallazgo**: `proveedor` (clave obligatoria de `kvg`) no tenía regla de normalización y conservaba los espacios múltiples del OCR; se agregó `"proveedor": NORM_TEXTO` y `CAMPOS_GENERICOS_CON_REGLA`. | team implementation | Hecho |
