...
| **Estado de diseño** | 🟢 T-301..T-305 implementados (motor R1-R7, evidencia, reglas raw, cadena contable y paridad verificada) |
...
| Subflujo contable: cadena 01 (centro de costo) → 02 (macro categoría) → 03 (concepto/código + condición impositiva) | §4.3 | E-CLAS-2 | F3 / T-304 | [x] hecho (`prompts_contable.py` + `contable.py`: contratos por paso, checkpoints con reanudación, `ErrorCadenaContable` con parciales y `clasificar_contable()` pura) |
...
| Paridad con `classification_pipeline.py` y `-M 11.1` de v1 | §8.2 (mapeo v1→v2) | E-CLAS | F3 / T-305 | [x] hecho (cadena contable **8/8**; exactitud de letra **v2 5/5** vs. **v1 2/5**; subconjunto en `tests/golden/F3/`) |

## 3. Definition of Design / contratos a congelar

- [x] Interfaz pública acordada: salida de tipo/letra con `candidatos_descartados`, `candidatos_restantes` y `reglas_aplicadas` (alineado a `Decision`/`CombinedEvidence`) y clasificación contable tipada (centro_costo, macro_categoria, concepto, codigo). — **T-303** (candidatos curados + `reglas_raw`/`candidatos_curados` en el detalle) y **T-304** (contratos por paso + `ClasificacionContableResult` con `condicion_impositiva`/`requiere_revision_humana`/`detalle`).
...
- [x] Casos de golden set / tests que lo validan: tests unitarios de reglas R1-R7 + casos por tipo/letra (A/B/C/M/E, exportación, conflicto R7) y de la cadena contable 01→02→03 (incluye default CC0006); paridad con v1 (T-305). — **T-305** (subconjunto `tests/golden/F3/` con casos sintéticos deterministas + casos reales con evidencia objetiva).
...
| 2026-09-10 | T-305 implementado (cierra el DoD de F3): **paridad medida con v1 sobre documentos y modelos reales**. La cadena contable coincide en **8/8** campos (2 casos × centro/macro/concepto/código) y, sobre los 5 casos con letra etiquetada, la exactitud de v2 es **5/5** frente a **2/5** de v1 (que no concluyó la letra en 4). **Bug encontrado y corregido**: el patrón de R5 portado literal del WIP usaba `\s+`, que cruzaba el **salto de línea** del markdown de Docling y capturaba la letra de la línea siguiente (`"FACTURA\n  Código: 1"` → `C` sobre dos PDFs reales cuyo encabezado dice `FACTURA A`). El defecto sobrevivió a T-301 porque **en v1 ese patrón nunca se ejecutó como código** (era un `criterio` descriptivo del prompt); se corrigió a `[ \t]+` + `\b`, conservando la semántica, con test de regresión. | team implementation | Hecho |
