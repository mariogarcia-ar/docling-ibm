...
| **Clase documental** (`es_comprobante`) | Qué **es** un documento, antes de qué dice: `comprobante` / `no_comprobante` / `indeterminado` (vocabulario `ClaseDocumento` del contrato F0). Es una **lectura**, no una decisión: no dice si el comprobante *sirve* para el gasto (`comprobante_valido`, que resuelve F5) ni *qué* comprobante es (`tipo_comprobante`). El **gate** de F2 (T-202) la decide sobre una vista barata antes de gastar en extracción, y el **laboratorio** de LLM externos la lee sobre el documento. ⚠️ Es el mismo campo y el mismo vocabulario en las dos puntas **a propósito**: con dos definiciones, comparar lab ↔ pipeline compararía cosas distintas sin fallar. |
